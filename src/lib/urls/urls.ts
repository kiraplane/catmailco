...
export const CANONICAL_BASE_URL = 'https://www.catmailco.org';
...
function getCatMailCoBaseUrl(url?: string) {
  ...
  try {
    ...
    if (
      parsedUrl.hostname === 'catmailco.org' ||
      parsedUrl.hostname === 'www.catmailco.org'
    ) {
...
export function getBaseUrl(): string {
  ...
  const catMailCoBaseUrl = getCatMailCoBaseUrl(configuredBaseUrl);
  if (catMailCoBaseUrl) {
    return catMailCoBaseUrl;
  }
