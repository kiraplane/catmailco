...
import { getGuideArticleUi, getHomeContent } from '@/data/catmailco/localized';
import { siteFacts } from '@/data/catmailco/sources';
import type { Guide } from '@/data/catmailco/types';
...
export function GuideArticle({
  ...
  return (
    <div className="bg-[#23182A] py-10 text-[#FFF8E7]">
      <JsonLd data={jsonLd} />
      <AdsterraSideRails />
      <Container className="px-4">
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_264px]">
          <article className="min-w-0 space-y-6">
            <header className="rounded-lg border border-[#6B536F] bg-[#33233B] p-5 md:p-7">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge
                  variant="outline"
                  className="border-[#FFB35C]/60 bg-[#23182A]/60 text-[#FFF8E7]"
                >
                ...
                {guide.tags.map((tag) => (
                  <Badge
                    key={tag}
                    variant="outline"
                    className="border-[#FFB35C]/40 bg-[#23182A]/50 text-[#FFF8E7]"
                  >
                    {tag}
                  </Badge>
                ))}
            ...
            <div className="overflow-hidden rounded-lg border border-[#6B536F] bg-[#33233B]">
              <div className="relative aspect-video w-full min-h-[180px] border-[#6B536F] border-b sm:aspect-[16/7]">
                <Image
              ...
              <div className="p-5 md:p-8">
                  ...
                  {guide.body.map((section, sectionIndex) => (
                    <div key={section.heading} className="contents">
                      <section id={toSectionId(section.heading)}>
                        ...
                        <div className="mt-3 space-y-4 text-base leading-8 text-[#D7C8D8]">
                        ...
                        {section.bullets ? (
                          <ul className="mt-4 grid gap-2 text-sm leading-7 text-[#D7C8D8]">
                            {section.bullets.map((bullet, index) => (
                              <li
                                key={`${section.heading}-bullet-${index}`}
                                className="rounded-md border border-[#6B536F] bg-[#23182A] px-3 py-2"
                              >
                                {bullet}
                              </li>
                            ))}
                      ...
                      {sectionIndex === 1 && guide.video ? (
                        <section className="overflow-hidden rounded-lg border border-[#6B536F] bg-black">
                          <iframe
                            ...
                            allowFullScreen
                          />
                          <div className="bg-[#23182A] px-4 py-3 text-sm leading-6 text-[#D7C8D8]">
                            {ui.videoPrefix}{' '}
                            <a
                ...
                <section className="mt-10 rounded-lg border border-[#6B536F] bg-[#23182A] p-5">
                  <div className="flex gap-3">
                    <ShieldCheck className="mt-1 size-5 shrink-0 text-[#7AD6CC]" />
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-7 text-[#D7C8D8]">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-[#FFB35C] underline underline-offset-4"
                        >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {nextRoutes.map((route) => (
                      <Button
                        ...
                        asChild
                        variant="outline"
                        className="h-auto min-w-0 justify-between gap-3 whitespace-normal border-[#6B536F] bg-[#33233B] px-3 py-3 text-left text-[#FFF8E7] hover:bg-[#402C48]"
                      >
