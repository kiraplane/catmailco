...
import type { Guide } from '@/data/moonlightpeaks/types';
          ...
          <article className="min-w-0 space-y-6">
            <header className="rounded-lg border border-[#4B315F] bg-[#1D102A] p-5 md:p-7">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge
                  variant="outline"
                  className="border-[#C77DFF]/60 bg-[#120719]/60 text-[#F4EAFE]"
                >
                ...
                {guide.tags.map((tag) => (
                  <Badge
                    key={tag}
                    variant="outline"
                    className="border-[#C77DFF]/40 bg-[#120719]/50 text-[#F4EAFE]"
                  >
                    {tag}
                  </Badge>
                ))}
            ...
            <div className="overflow-hidden rounded-lg border border-[#4B315F] bg-[#1D102A]">
              <div className="relative aspect-[16/7] min-h-[180px] border-[#4B315F] border-b">
                <Image
              ...
              <div className="p-5 md:p-8">
                {guide.video ? (
                  <section className="mb-8 overflow-hidden rounded-lg border border-[#4B315F] bg-black">
                    <iframe
                      ...
                      allowFullScreen
                    />
                    <div className="bg-[#120719] px-4 py-3 text-sm leading-6 text-[#DED2F6]">
                      {ui.videoPrefix}{' '}
                      <a
                ...
                <div className="space-y-8">
                  ...
                  {guide.body.map((section) => (
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-4 text-base leading-8 text-[#DED2F6]">
                        {section.paragraphs.map((paragraph, index) => (
                          <p key={`${section.heading}-${index}`}>{paragraph}</p>
                        ))}
                      </div>
                      {section.bullets ? (
                        <ul className="mt-4 grid gap-2 text-sm leading-7 text-[#DED2F6]">
                          {section.bullets.map((bullet, index) => (
                            <li
                              key={`${section.heading}-bullet-${index}`}
                              className="rounded-md border border-[#4B315F] bg-[#120719] px-3 py-2"
                            >
                              {bullet}
                            </li>
                          ))}
                ...
                <section className="mt-10 rounded-lg border border-[#4B315F] bg-[#120719] p-5">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-7 text-[#DED2F6]">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-[#C77DFF] underline underline-offset-4"
                        >
                ...
                <section className="mt-10 rounded-lg border border-[#4B315F] bg-[#120719] p-5">
                  <div className="flex flex-wrap items-end justify-between gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className="border-[#C77DFF]/50 text-[#F4EAFE]"
                    >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {nextRoutes.map((route) => (
                      <Button
                        ...
                        asChild
                        variant="outline"
                        className="h-auto min-w-0 justify-between gap-3 whitespace-normal border-[#4B315F] bg-[#1D102A] px-3 py-3 text-left text-[#F4EAFE] hover:bg-[#2B1838]"
                      >
